...
DNGN.Mob.prototype._draw = function() {};

// All of our Mob subclasses!

DNGN.Mob.Player = function(point, base) {
    DNGN.Mob.call(this, point, base);
...
DNGN.Mob.Player.extend(DNGN.Mob);
// THIS ^^^^^ does this:
//DNGN.Mob.Player.prototype = new DNGN.Mob();
//DNGN.Mob.Player.prototype.constructor = DNGN.Mob.Player;



/*
DNGN.Mob.Player.prototype.getHP =  function() {
    return " "+this._curHP+"/"+this._maxHP;
};
*/

DNGN.Mob.Player.prototype.act = function() {
    //Game.engine.lock();
    DNGN.engine.wait();
...
/*
DNGN.Mob.Player.prototype.getStats = function() {
    var someStats = [];
...
DNGN.Mob.Player.prototype.wipeMessage = function() {
    var wipe = 0;
    for (i=0; i<80; i++) {
        DNGN.display.draw(wipe, 27, " ");
        wipe++;
    }
};
*/

DNGN.Mob.Player.prototype.handleEvent = function(e) {
    // process user input
    ...
    if (!(newKey in DNGN.map)) { return; } // can't move in this direction
    if (DNGN.map[newKey].walk === false) { return; } // can't walk here

    DNGN.display.draw(this._x, this._y, DNGN.map[this.getXY()].ch, DNGN.map[this.getXY()].fg);
    this._x = newX;
    this._y = newY;
    this._draw();
...
/*
DNGN.Player.prototype._checkBox = function() {
    var key =  this.getXY();
    if (DNGN.map[key] != "*") {
       DNGN.updateLog("There is no box at ["+this._x+", "+this._y+"]!");
    } else if (key == Game.ananas) {
        DNGN.updateLog("Hooray! You found an ananas and won this game.");
        //Game.engine.lock();
        DNGN.engine.gameover();
        window.removeEventListener("keydown", this);
    } else {
        DNGN.updateLog("The box at ["+this._x+", "+this._y+"] is empty! D:");
    }
};
*/
DNGN.Mob.Player.prototype._draw = function() {
    DNGN.display.draw(this._x, this._y, this._sym, this._color);
